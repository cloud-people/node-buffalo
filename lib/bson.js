...
exports.decode = function(buffer, offset, includeUndefined) {
    return decode(buffer, offset || 0, false, includeUndefined)
}
...
function decode(buffer, offset, isArray, includeUndefined) {
    var length = readInt()
    ...
    while (true) {
        ...
        switch (type) {
            ...
            case SYMBOL_TYPE:
                var str = readString()
                append(name, JSON.stringify(str));
                break
            case EMBEDDED_DOCUMENT_TYPE:
    ...
    function readEmbeddedObject(isArray) {
        var embeddedLength = readInt()
        offset -= 4 // rewind int size (since parse will read it again)
        var str = decode(buffer, offset, isArray, includeUndefined)
        offset += embeddedLength
        return str
    }
