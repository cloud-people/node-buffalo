...
function parse(buffer, offset, isArray) {
    ...
    return deserialize(buffer, offset, isArray, object, function (obj, prop, value) {
        obj[prop] = value
    }, parse)
}
...
function decode(buffer, offset, isArray) {
    ...
    deserialize(buffer, offset || 0, isArray, sb, function (sb, prop, value) {
         sb.append(`"${prop}":${value},`);
    }, decode)
    sb.replace(sb.length() - 1, isArray ? ']' : '}');
}

function deserialize(buffer, offset, isArray, object, cons, wrapperFn) {
    var length = readInt()
