...
function parse(buffer, offset, isArray) {
    ...
    return doParse(buffer, offset, isArray, object, function (obj, prop, value) {
        obj[prop] = value
    }, parse)
}

function decode(buffer, offset, isArray) {
    var sb = new StringBuilder(isArray ? '[' : '{');
    doDecode(buffer, offset || 0, isArray, sb, function (sb, prop, value) {
         sb.append(isArray: `${value},` : `"${prop}":${value},`);
    }, decode)
    sb.replace(sb.length() - 1, isArray ? ']' : '}');

    return sb.toString();
}

function doParse(buffer, offset, isArray) {
    var length = readInt()
    ...
    while (true) {
        ...
        switch (type) {
            case FLOAT_TYPE:
                cons(object, name, IEEE754.readIEEE754(buffer, offset, 'little', 52, 8))
                offset += 8
                break
            case INT32_TYPE:
                cons(object, name, readInt())
                break
            case INT64_TYPE:
                cons(object, name, new Long(readInt(), readInt()))
                break
            case STRING_TYPE:    
            case SYMBOL_TYPE:
                cons(object, name, readString())
                break
            case EMBEDDED_DOCUMENT_TYPE:
                cons(object, name, readEmbeddedObject(false))
                break
            case ARRAY_TYPE:
                cons(object, name, readEmbeddedObject(true))
                break
            case BINARY_TYPE:
                ...
                cons(object, name, binary)
                break
            case OBJECT_ID_TYPE:
                var objectIdBuffer = new Buffer(12)
                buffer.copy(objectIdBuffer, 0, offset, offset +=12)
                cons(object, name, new ObjectId(objectIdBuffer))
                break
            case BOOLEAN_TYPE:
                cons(object, name, buffer[offset++] == 1)
                break
            case DATE_TIME_TYPE:
                cons(object, name, new Date(new Long(readInt(), readInt()).toNumber()))
                break
            case REG_EXP_TYPE:
                cons(object, name, new RegExp(readCString(), readCString()))
                break
            case DB_REF_TYPE:
                var dbrefName = readString()
                var dbrefObjectId = new Buffer(12)
                buffer.copy(dbrefObjectId, 0, offset, offset +=12)
                cons(object, name, new DBRef(dbrefName, new ObjectId(dbrefObjectId)))
                break
            case CODE_TYPE:
                cons(object, name, makeFunction(readString()))
                break
            case CODE_WITH_SCOPE_TYPE:
                readInt() // ignoring the size, we don't need it
                cons(object, name, makeFunction(readString(), readEmbeddedObject(cons)))
                break
            case TIMESTAMP_TYPE:
                cons(object, name, new Timestamp(readInt(), readInt()))
                break
            case UNDEFINED_TYPE:
                cons(object, name, undefined)
                break
            case NULL_TYPE:
            case MIN_KEY:
            case MAX_KEY:
                cons(object, name, null)
                break
            default:                
    ...
    function readEmbeddedObject(isArray) {
        ...
        var object = wrapperFn(buffer, offset, isArray)
...
function doDecode(buffer, offset, isArray, object, cons, wrapperFn) {
    var length = readInt()
    ...
    while (true) {
        ...
        switch (type) {
            ...
            case INT32_TYPE:
                ...
                break
            case INT64_TYPE:
                cons(object, name, new Long(readInt(), readInt()).toString(10))
                break
            case STRING_TYPE:    
            case SYMBOL_TYPE:
                cons(object, name, `"${readString()}"`)
            ...
            case BINARY_TYPE:
                ...
                buffer.copy(binary, 0, offset, offset += binaryLength)
                binary.subtype = subtype
                cons(object, name, binary.toString())
                break
            case OBJECT_ID_TYPE:
                var objectIdBuffer = new Buffer(12)
                buffer.copy(objectIdBuffer, 0, offset, offset +=12)
                cons(object, name, `"${new ObjectId(objectIdBuffer).toString()}"`)
                break
            case BOOLEAN_TYPE:
                cons(object, name, buffer[offset++] == 1)
                break
            case DATE_TIME_TYPE:
                cons(object, name, `"${new Date(new Long(readInt(), readInt()).toNumber()).toISOString()}"`)
                break
            case REG_EXP_TYPE:
                cons(object, name, new RegExp(readCString(), readCString()).toString())
                break
            case DB_REF_TYPE:
                var dbrefName = readString()
                var dbrefObjectId = new Buffer(12)
                buffer.copy(dbrefObjectId, 0, offset, offset +=12)
                cons(object, name, new DBRef(dbrefName, new ObjectId(dbrefObjectId)).toString())
                break
            case CODE_TYPE:
                cons(object, name, makeFunction(readString()).toString())
                break
            case CODE_WITH_SCOPE_TYPE:
                readInt() // ignoring the size, we don't need it
                cons(object, name, makeFunction(readString(), readEmbeddedObject(cons)).toString())
                break
            case TIMESTAMP_TYPE:
                cons(object, name, new Timestamp(readInt(), readInt()).toString())
                break
            case UNDEFINED_TYPE:
                cons(object, name, undefined)
                break
