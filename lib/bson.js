...
var printableCharacters = require ('printable-characters')
var strlen = printableCharacters.strlen;
var first = printableCharacters.first;
...
exports.decode = function(buffer, offset) {
    return decode(buffer, offset || 0, false)
}
...
function decode(buffer, offset, isArray) {
    ...
    var sb = isArray ? new StringBuilder('[') : new StringBuilder('{')
    ...
    while (true) {
        ...
        switch (type) {
            case FLOAT_TYPE:
                append(name, IEEE754.readIEEE754(buffer, offset, 'little', 52, 8))
                offset += 8
                break
            case INT32_TYPE:
                append(name, readInt())
                break
            case INT64_TYPE:
                append(name, new Long(readInt(), readInt()))
                break
            case STRING_TYPE:    
            case SYMBOL_TYPE:
                var str = readString()
                append(name, `"${jsStringEscape(first(str, strlen(str)))}"`)
                break
            case EMBEDDED_DOCUMENT_TYPE:
                append(name, readEmbeddedObject(false))
                break
            case ARRAY_TYPE:
                append(name, readEmbeddedObject(true))
                break
            case BINARY_TYPE:
                var binaryLength = readInt()
                var subtype = buffer[offset++]
                ...
                buffer.copy(binary, 0, offset, offset += binaryLength)
                binary.subtype = subtype
                append(name, binary.toString())
                break
            case OBJECT_ID_TYPE:
                var objectIdBuffer = new Buffer(12)
                buffer.copy(objectIdBuffer, 0, offset, offset +=12)
                append(name, `"${new ObjectId(objectIdBuffer).toString()}"`)
                break
            case BOOLEAN_TYPE:
                append(name, buffer[offset++] == 1)
                break
            case DATE_TIME_TYPE:
                append(name, `"${new Date(new Long(readInt(), readInt()).toNumber()).toISOString()}"`)
                break
            case REG_EXP_TYPE:
                append(name, new RegExp(readCString(), readCString()).toString())
                break
            case DB_REF_TYPE:
                var dbrefName = readString()
                var dbrefObjectId = new Buffer(12)
                buffer.copy(dbrefObjectId, 0, offset, offset +=12)
                append(name, new DBRef(dbrefName, new ObjectId(dbrefObjectId)).toString())
                break
            case CODE_TYPE:
                append(name, makeFunction(readString()))
                break
            case CODE_WITH_SCOPE_TYPE:
                readInt() // ignoring the size, we don't need it
                append(name, makeFunction(readString(), readEmbeddedObject()).toString())
                break
            case TIMESTAMP_TYPE:
                append(name, new Timestamp(readInt(), readInt()).toString())
                break
            case UNDEFINED_TYPE:
    ...
    function append(name, val) {
        ...
        sb.append(isArray ? `${val}` : `"${name}":${val}`)
        firstProperty = false
    }
    ...
    function readEmbeddedObject(isArray) {
        var embeddedLength = readInt()
        offset -= 4 // rewind int size (since parse will read it again)
        var str = decode(buffer, offset, isArray)
        offset += embeddedLength
        return str
    }
